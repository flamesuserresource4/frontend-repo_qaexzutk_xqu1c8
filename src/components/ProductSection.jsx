  ...
  return (
    <section className="py-16 bg-gradient-to-b from-rose-50/40 to-white" id="product">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-start">
        <div className="bg-white/80 border border-slate-200 rounded-3xl p-5">
          ...
          <div className="mt-4 grid grid-cols-4 gap-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="aspect-square rounded-xl bg-rose-50 border border-rose-100" />
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={()=> onAddToCart(product || { title:'Droomwoordjes', slug:'droomwoordjes', price:29.95 }, qty)} className="px-5 py-3 rounded-full bg-rose-300 text-slate-800 shadow border border-rose-200 hover:shadow-md">In winkelmand</button>
            <a href="#faq" className="px-5 py-3 rounded-full bg-white text-slate-700 border border-slate-200 hover:bg-slate-50">Meer info</a>
          </div>
        </div>
      </div>
    </section>
